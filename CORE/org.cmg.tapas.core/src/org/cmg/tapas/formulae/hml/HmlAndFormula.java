 ...
 *
 */
public final class HmlAndFormula<X extends Process<X, Y>,Y extends ActionInterface> 
						extends HmlFormula<X,Y> {

	private LinkedList<HmlFormula<X,Y>> elements;
	
	public HmlAndFormula( ) {
		this(new LinkedList<HmlFormula<X, Y>>());
	}
	
	public HmlAndFormula( HmlFormula<X,Y> left , HmlFormula<X,Y> right ) {
		this();
		this.elements.add(left);
		this.elements.add(right);
	}
	
	public HmlAndFormula(LinkedList<HmlFormula<X, Y>> elements) {
		this.elements = elements;
	}
	
	public HmlAndFormula(Collection<HmlFormula<X, Y>> elements) {
	...
	public Proof<X, Y> getProof( X s ) {
		...
		for (HmlFormula<X,Y> f : elements) {
			p = f.getProof(s);
			if (!p.isSuccess()) {
				return new Proof<X,Y>( s , this , new Object[] { p } ,  false );
			}
			proofs.add(p);
		}
		return new Proof<X,Y>( s , this , proofs.toArray() );
	}
	...
	protected HmlFormula<X, Y> doClone() {
		...
		return new HmlAndFormula<X, Y>(foo);
	}
	...
	public void addAllSubFormulae( Collection<HmlFormula<X,Y>> c ) {
		elements.addAll(c);
	}
	...
	protected boolean _equals(Object obj) {
		...
		if (obj instanceof HmlAndFormula) {
			HmlAndFormula<X, Y> f = (HmlAndFormula<X, Y>) obj;
