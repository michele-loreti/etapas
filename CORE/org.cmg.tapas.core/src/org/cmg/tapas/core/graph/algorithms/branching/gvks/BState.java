...
> {
    ...
    // transizioni inerti che hanno origine in questo stato
...
//    }
    ...
    void addInert(BState<S,A> to) {    	
    	if( inert.add(to) && inert.size()==1 )
    		clazz.decreaseLastCount();    		
    }
    ...
    /** Marca lo stato se si tratta di uno stato ultimo. **/
    void markIfLast() {
    	if( inert.isEmpty() && !mark ) {
    		mark = true;
    		clazz.increaseLastMarkedCount();    		
    	}
    }
