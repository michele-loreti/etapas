...
/** 
 * GVKS2Checker offre i metodi per il calcolo
...
> implements EquivalenceSolver<S,A> {	
	...
	/** 
	 * Costruisce un GVKS2Checker per il grafo specificato. 
	 **/
	public GVKS2Checker(
		GraphData<S,A> graph
	) {
	...
	/**
	 * Costruisce un GVKS2Checker per il grafo 
	...
	private <V> Evaluator<S,BClazz<S,A>> computeEquivalence(
		...
		System.out.println( "gvks2 cycles: " + cycles );

		return new Evaluator<S,BClazz<S,A>>(){
            @Override 
            public BClazz<S,A> eval(S state) {
	...
	private void refine(BState<S,A> state1, BState<S,A> state2) {
		...
		while( !working.isEmpty() ) {
			...
			while( iter.hasNext() ) {
				...
				lastChanged |= split(
					action, splitter, elements, sourceBlocks, 
					index, !iter.hasNext()
				);
				...
				for( BClazz<S,A> clazz: sourceBlocks ) {					
					...
					if( clazz.isEmpty() ){
						...
						queue.remove(clazz);
						queue.add( sibling );
						sibling.stealIndex(clazz);			
						if( !iter.hasNext() && !splitted )
							sibling.addStable(index);
					} else {
	...
	private boolean split( 
		...
		for( BState<S,A> state: elements ) {							  
			...
			for( BState<S,A> preState: preset ) {	 	
				...
				if( clazz.getFlag() || clazz.isStableWrt(index) )						
					continue;										
		...
		for( BClazz<S,A> touched: clazzes ) {
			boolean localLastChanged = false;
			for( BState<S,A> state: touched.getSibling() ) { 
				...
				localLastChanged |= nonlast1!=nonlast2;
			}
			lastChanged |= localLastChanged;
			if( localLastChanged )
				touched.getSibling().clearStable();
			else if( lastAction )
				touched.getSibling().addStable(index);
		}
