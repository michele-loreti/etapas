...
/** 
 * GVSChecker offre i metodi per il calcolo
 * della bisimulazione branching su grafi poggiando
 * sull'algortimo di Groote-Vaandrager semplificato.
 * <p>
...
> implements EquivalenceSolver<S,A> {	
	...
	/**
	 * Costruisce un BranchingChecker per il grafo 
	...
	private int split( 
		...
		for( BBlock<S,A> block: sourceBlocks ) {
			...
			// for every state in the block hitting the splitter
			boolean hit1;
			LinkedList<BState<S,A>> hit2;
			for( BState<S,A> bstate: block ) {
				hit1 = bstate.isMarked();						
				hit2 = null;						
				if( !bstate.getInert().isEmpty() ) {
				...
				if( hit2==null) {
					(hit1 ? block1 : block2).addLast(bstate);
					continue;
				}
				if( !hit1 )	{
